...
- `assets/` — `logo.png` is the project art; `logo.tga` (addon list icon)
  ...
  `expired.ogg`, `dead.ogg`, and `distance.ogg` are the alert sounds,
  "totem expired", "totem dead", and "totem distance" from one of
...
the totem had left: `assets/expired.ogg` ("totem expired") if it had
...
- Alert icon side, left or right (left by default): which side of the
  HUD the "!" hangs off
- Play a sound when a totem expires ("totem expired") or is killed
